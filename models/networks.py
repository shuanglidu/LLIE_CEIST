...
def define_attUnet(input_attnc, output_attnc, which_model_net1, gpu_ids=[]):
    use_gpu = len(gpu_ids) > 0
    if use_gpu:
        assert(torch.cuda.is_available())
    if which_model_net1 == 'unet':
        netAtt = Unet_att(input_attnc, output_attnc)
    else:
        raise NotImplementedError('Attention map Generator model name [%s] is not recognized' % which_model_net1)
    if len(gpu_ids) >= 0:
        netAtt.cuda(device=gpu_ids[0])
        netAtt = torch.nn.DataParallel(netAtt, gpu_ids)
    return netAtt
...
def MSELoss(predict, target):
    ...
    # loss = 0.85 * F.l1_loss(predict, target) + 0.15 * F.mse_loss(predict, target) + 0.85 * ssim(predict, target, data_range=1, size_average=True)
    loss = 5 * F.l1_loss(predict, target)
    return loss

def L1Loss(out_image, gt_image):
    loss = F.l1_loss(out_image, gt_image)
    return loss
...
def conct(x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
    # [N, C, H, W]
...
class Unet_att(nn.Module):
    def __init__(self, input_attnc, output_attnc):
        super(Unet_att, self).__init__()

        self.input_attnc = input_attnc
        self.output_attnc = output_attnc
        self.conv1 = DoubleConv(input_attnc, 64)
        ...
        self.conv10 = nn.Conv2d(64, output_attnc, 1)

    def forward(self, attlow):
        c1 = self.conv1(attlow)
        p1 = self.pool1(c1)
        c2 = self.conv2(p1)
        p2 = self.pool2(c2)
        c3 = self.conv3(p2)
        ...
        c5 = self.conv5(p4)
        up_6 = self.up6(c5)
        merge6 = conct(up_6, c4)
        c6 = self.conv6(merge6)
        up_7 = self.up7(c6)
        merge7 = conct(up_7, c3)
        c7 = self.conv7(merge7)
        up_8 = self.up8(c7)
        merge8 = conct(up_8, c2)
        c8 = self.conv8(merge8)
        up_9 = self.up9(c8)
        merge9 = conct(up_9, c1)
...
class GrayEdgeAttentionUNet(nn.Module):
    def __init__(self):
        ...
        self.up6 = nn.ConvTranspose2d(512, 256, kernel_size=2, stride=2, output_padding=(0,1))
...
class Unet_resize_conv(nn.Module):
    ...
    def forward(self, input, gray):
        ...
        if self.opt.use_norm == 1:
            ...
            conv8 = F.upsample(conv8, scale_factor=2, mode='bilinear')  #修改了 scale_factor 原来为2
